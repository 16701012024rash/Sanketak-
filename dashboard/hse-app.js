(function () {
            ...
            <aside class="sidebar">
                ...
                <div class="system-card">
                    ...
                    <div>
                        ...
                        <small>Worker safety reports</small>
                    </div>
                </div>
            </aside>
            <main class="main">
    ...
    function renderDashboard(content) {
        ...
        // Barrier drift comes from GET /intelligence/{id}, one request per
        // report. Rather than fire N requests to fill this panel, it stays
        // empty and says why.
        const driftAvailable = services.getConnectionState().driftAvailable;
        const drift = driftAvailable ? services.getBarrierDriftConcerns() : [];

        content.innerHTML = `
            ${connectionNotice()}
            <div class="kpi-grid five">
                ${kpiCard("Reports Received", summary.reportsReceived, "Worker safety reports received", "info")}
            ...
            <div class="two-column">
                <section class="panel">
                    ...
                    <div class="stack">
                        ${driftAvailable
                            ? (drift.map(driftCard).join("") || emptyState("No repeated barrier weakening has been detected."))
                            : emptyState("Barrier drift is not shown here. It is available per report, from the report detail page.")}
                    </div>
                </section>
                ...
                <section class="panel">
                    <div class="panel-header">
                        <div>
                            ...
                            <p>Site safety intelligence from current reports.</p>
                        </div>
                    </div>
                    <div class="site-grid">
    ...
    async function renderReport(content) {
        ...
        if (!report) {
            content.innerHTML = `
                ${connectionNotice()}
                ${emptyState("No report is available yet. Once workers submit reports from the mobile app, they will appear here.")}
            `;
            return;
        }
        ...
        content.innerHTML = `
            ...
            <section class="panel">
                ...
                <div class="evidence-grid">
                    ${report.evidence.length ? report.evidence.map(function (item) {
                        return `
                            <article class="evidence-card">
                                <q>${esc(item.text)}</q>
                                ${item.type || item.interpretation
                                    ? `
                                        <div class="mapped-row">
                                            ${item.type ? `<span class="tag info">${esc(item.type)}</span>` : ""}
                                            ${item.interpretation ? `<strong>${esc(item.interpretation)}</strong>` : ""}
                                        </div>
                                    `
                                    : ""}
                            </article>
                        `;
                    }).join("") : emptyState("No evidence spans were extracted from this report.")}
                </div>
            </section>
            ...
            <div class="two-column">
                <section class="panel">
                    <div class="panel-header">
                        <div>
                            ...
                            <p>Reports with a similar precursor pattern, ranked by match score.</p>
                        </div>
                    </div>
                    <div class="stack" id="relatedSignalsPanel">
                        <p class="muted">Loading related signals...</p>
                    </div>
                </section>
                ...
                <section class="panel">
                    ...
                    <div id="barrierDriftPanel">
                        <p class="muted">Loading barrier drift...</p>
                    </div>
                </section>
            </div>
            ...
            <section class="panel">
                ...
                <div class="precedent-grid" id="precedentPanel">
                    <p class="muted">Loading precedents...</p>
                </div>
            </section>
        ...
        await fillReportIntelligence(report);
        ...
        content.innerHTML = `
            ...
            <section class="panel narrow-panel">
                <div class="panel-header">
                    <div>
                        <h2>Safety Alerts</h2>
                        <p>Dashboard settings shown in the browser. These are not persisted; the backend has no settings endpoint.</p>
                    </div>
                </div>
    ...
    function renderPdfExport(content) {
        ...
        if (!reports.length) {
            content.innerHTML = `
                ${connectionNotice()}
                ${emptyState("No reports are available to export yet.")}
            `;
            return;
        }
            ...
            <section class="panel">
                <div class="panel-header">
                    <div>
                        ...
                        <p>Loading worker safety reports and corrective actions.</p>
                    </div>
                </div>
            </section>
        `;
    ...
    function connectionPanel(state) {
        ...
        return `
            <section class="panel connection-panel">
                <div class="panel-header">
                    <div>
                        <h2>${needsLogin ? "Dashboard Login Required" : "Backend Setup Required"}</h2>
                        <p>${esc(state.error || "The dashboard could not reach the backend.")}</p>
                    </div>
                </div>
                ${needsLogin
                    ? `
                        <p class="muted">
                            Workers submit reports without logging in, but every dashboard read
                            requires a signed-in HSE staff account.
                        </p>
                        <a class="primary-button" href="login.html">Go to Login</a>
                    `
                    : `
                        <p class="muted">
                            Check <strong>supabase-config.js</strong> for the backend URL, and make sure
            ...
            <section class="panel connection-panel warning">
                <div class="panel-header">
                    <div>
                        ...
                        <p>Some data could not be read from the backend.</p>
                    </div>
                </div>
                <ul>
    ...
    // Loads GET /intelligence/{id} and GET /precedents/{id} for the one
    // report on screen, then fills the three panels that depend on them.
    async function fillReportIntelligence(report) {
        const relatedPanel = document.getElementById("relatedSignalsPanel");
        const driftPanel = document.getElementById("barrierDriftPanel");
        const precedentPanel = document.getElementById("precedentPanel");

        if (!relatedPanel || !driftPanel || !precedentPanel) {
            return;
        }
        ...
        try {
            intelligence = await services.loadReportIntelligence(report.id);
        } catch (error) {
            const message = `<p class="muted">Could not load intelligence: ${esc(error.message)}</p>`;
            relatedPanel.innerHTML = message;
            driftPanel.innerHTML = message;
            precedentPanel.innerHTML = message;
            return;
        }

        relatedPanel.innerHTML = relatedSignalsMarkup(intelligence);
        driftPanel.innerHTML = driftMarkup(intelligence);
        precedentPanel.innerHTML = precedentMarkup(intelligence);
    }

    function relatedSignalsMarkup(intelligence) {
        if (intelligence.error) {
            return `<p class="muted">Related signals unavailable: ${esc(intelligence.error)}</p>`;
        }

        if (!intelligence.relatedSignals.length) {
            return `<p class="muted">No report with a similar precursor pattern was found.</p>`;
        }

        return intelligence.relatedSignals.map(function (signal) {
            const location = signal.site + " / " + signal.area;

            // A match the current list does not contain (filtered out, or
            // added since the last refresh) is still shown, by id.
            const body = `
                <strong>${esc(signal.trackingToken)}</strong>
                <span>${esc(signal.resolved ? location : "Not in the current list")}</span>
                <span>${esc(signal.barrierFailure)}</span>
                <small>${signal.matchScore}% match${signal.resolved ? " - " + relativeTime(signal.receivedAt) : ""}</small>
            `;

            if (!signal.resolved) {
                return `<div class="signal-row">${body}</div>`;
            }

            return `
                <a class="signal-row" href="report.html?id=${encodeURIComponent(signal.trackingToken)}">
                    ${body}
                </a>
            `;
        }).join("");
    ...
    function driftMarkup(intelligence) {
        if (intelligence.error) {
            return `<p class="muted">Barrier drift unavailable: ${esc(intelligence.error)}</p>`;
        }

        const slice = intelligence.driftSlice || {};
        const sliceNote = slice.sliceable === false
            ? `<p class="muted">This report does not state the activity or hazard to group on, so no drift slice could be formed. That is not the same as a clean slice.</p>`
            : `<p class="muted">Compared against ${Number(slice.reports_in_slice || 0)} reports sharing this activity and hazard.</p>`;

        const driftRows = intelligence.drift.length
            ? intelligence.drift.map(function (row) {
                return `
                    <article class="drift-card">
                        <div>
                            <span class="eyebrow">Barrier</span>
                            <h3>${esc(row.barrier)}</h3>
                        </div>
                        <div class="metric-grid">
                            ${miniMetric("Failure mode", row.failureMode)}
                            ${miniMetric("Occurrences", String(row.occurrences))}
                            ${miniMetric("Status", cap(row.riskStatus))}
                        </div>
                    </article>
                `;
            }).join("")
            : `<p class="muted">No barrier has failed repeatedly within this slice.</p>`;

        const emerging = intelligence.emergingRisks.length
            ? `
                <div class="stack">
                    <span class="eyebrow">Emerging risks</span>
                    ${intelligence.emergingRisks.map(function (row) {
                        return `
                            <article class="drift-card">
                                <div>
                                    <span class="eyebrow">Barrier</span>
                                ...
                                <div class="metric-grid">
                                    ${miniMetric("Failure mode", row.failureMode)}
                                    ${miniMetric("Recent", String(row.recentOccurrences))}
                                    ${miniMetric("Baseline", String(row.baselineOccurrences))}
                                    ${miniMetric("Rate ratio", row.rateRatio + "x")}
                                </div>
                            </article>
                        `;
                    }).join("")}
        ...
        return sliceNote + driftRows + emerging;
    }

    function precedentMarkup(intelligence) {
        if (!intelligence.precedentsAvailable) {
            return `<p class="muted">Historical precedents could not be loaded.</p>`;
        }

        if (!intelligence.precedents.length) {
            return `<p class="muted">No historical precedent matched this report.</p>`;
        }

        // Name, description and source are all the backend records. No
        // similarity score, year, matching factors or key lesson exists.
        return intelligence.precedents.map(function (precedent) {
            return `
                <article class="precedent-card">
                    <h3>${esc(precedent.title)}</h3>
                    <p>${esc(precedent.description)}</p>
                    <small>Source: ${esc(precedent.source)}</small>
                </article>
            `;
        }).join("");
    }
